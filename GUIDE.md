...
3. 수임처별 순차 처리:
   ...
   - **기간 설정**: GUI 연도/월 반영 (미선택 시 전월 자동 계산). 매월 → 해당월, 반기 → 상/하반기 자동 판단
...
3. **"전체실행" 클릭 시 툴바 비밀번호 필드에서 읽기** (Phase 7 선택 시만 툴바에 표시)
4. 수임처별 순차 처리:
   - **WEHAGO 메인 복귀** → 사업자번호 검색 → 수임처명 fallback
   ...
   - **WehagoNTS 폴더 선택**: COM UIAutomation으로 저장 폴더 지정 + 파일 자동 이동
