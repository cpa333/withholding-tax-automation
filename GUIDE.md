...
> **신고주기(report_cycle):** `clients.report_cycle` 컬럼(SCHEMA v3). "새로 가져오기" 시 스크램값으로 채워지며, GUI 수임처 테이블의 **주기 컬럼 드롭다운**(매월/반기/빈)으로 수동 보정 가능(`update_report_cycle`). 반기 수임처는 원천징수를 6·12월에만 신고하므로 향후 월별 배치에서 처리 대상 필터링에 활용 가능.
...
3. 수임처별 순차 처리:
   ...
   - **기간 설정**:
     ...
     - **반기** → **유저 입력월 기준**. `compute_half_period()`: 6월 입력→당해 1~6월(상반기) / 12월 입력→당해 7~12월(하반기). 반기 신고는 연 2회이며 6·12월에만 실행(그 외 월은 마감 스킵, 라디오 확정값 역충전은 허용).
   - **조회 → 마감/마감해제**: 미마감이면 마감 적용(2단계 모달: 유의사항 → 마감완료). 이미 마감이면 **마감해제 → (같은 월) 재조회 → 재마감** 사이클로 동작 — 마감해제(마감 취소) 후 동일한 확인 모달 패턴 처리, 버튼이 "마감해제" → "마감" 으로 전환되었는지 검증한 뒤, 같은 월로 재조회하여 새 내용을 다시 불러오고 재마감을 적용. 재조회 후 마감 버튼이 정상 렌더되지 않으면 재마감 불가로 간주. 각 단계에서 상태가 예상과 다르면 `RuntimeError` 로 해당 잡을 실패 처리(어댑터 `wehago_swta.py`의 try/except 경유).
     - 조회·마감 적용 로직은 헬퍼(`_click_search_and_dismiss`, `_apply_close`, `_wait_close_button`)로 추출되어 최초 진입과 마감해제 후 재처리 양쪽에서 재사용.
...
Phase 9(SWER0101)에서 생성된 `.01` 파일을 홈택스에 업로드하여 파일변환신고 수행. 포털이 다르므로(WEHAGO → 홈택스) 독립 세션에서 실행.

**상태: 라이브 검증 완료 (2026-07-17)** — 실제 제출 E2E 성공 + 멀티 수임처 루프(3회 연속 재진입) 3/3 성공. 동일 과세기간 재제출은 신고내역에서 최신 접수번호로 **대체**됨(누적 아님). 상세 이력은 `src/automation/hometax/PROGRESS.md`.

1. Chrome 실행 → 홈택스 접속
2. 사용자 공동/금융인증서 수동 로그인
3. 수임처별 순차 처리 (페이지 리셋 없이 이전 수임처의 종료 화면에서 재진입):
   - **Phase 9 결과물 검색**: `~/Desktop/원천전자신고_YYYYMM/{수임처명}/*.01`에서 최신 파일 탐색. 파일이 없으면 해당 수임처만 실패로 기록하고 다음 수임처 계속 — 사유는 로그 패널에 `[단계 실패]`로 표시
   ...
   - **전자파일 비밀번호 입력**: 검증 직후 팝업에 Phase 9 제작 시 설정한 비밀번호 주입. 다건 실행 시 툴바 비밀번호 1개가 전체 수임처에 공통 적용되므로 수임처별 비밀번호 통일 필요
   - **제출**: 제출하러 가기 → '전자파일 제출하기' **real click** → 확인 모달 2회 → 접수증. dry-run 체크(기본) 시 제출 화면 진입까지만
4. 세션 연장: 20분 주기 `$c.pp.sessionXtn($p)` + `sessionTimer("Y")` 자동 호출
5. 접수 확정 확인: 접수증 팝업 건수 로그는 0건 오탐 레이스가 있어 참고용 — **신고내역 조회(접수증·납부서)** 그리드의 접수일시·접수번호가 신뢰 경로
