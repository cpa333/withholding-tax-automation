...
> 표시되는 번호 = phase_id. phase 2는 병렬(NHIS+NPS 동시) 전용 메타데이터.

| Phase | 포털 | 기능 | 상태 |
|-------|------|------|------|
| 1 | WEHAGO | 수임처 리스트 확보 | 완료 |
| 2 | (병렬) | 공단 EDI 병렬 자동화 (NHIS+NPS) | 완료 |
| 3 | 국민건강보험 EDI | 결정내역 PDF/Excel 다운로드 | 완료 |
| 4 | 국민연금 EDI | 결정내역 PDF/Excel 다운로드 | 완료 |
| 5 | 고용보험 EDI | 고용보험료 지원금 정보 인쇄물 다운로드 | 프로토타입(라이브 튜닝 필요) |
...
### Phase 10: 고용보험 EDI (근로복지공단)

엑셀 v3 (C86~H106) 워크플로우 기반. 위하고 급여자료입력 병합(실업급여지원금/환수금
반영)은 다음 작업으로 분리 — 본 phase는 **raw data(고용보험료 지원금 정보 인쇄물)
다운로드까지만** 담당. 현재 단독(직렬) 메뉴이며, 병렬 phase 2 편입은 추후.

> **상태: 프로토타입** — 포털 DOM/셀렉터가 라이브 튜닝 전이므로 메뉴 이동/인쇄 버튼
> 클릭 로직은 HTML 기반 추정값(`_click_text_in_menu`, 텍스트 매칭). 라이브 테스트로
> 실제 DOM을 확인한 뒤 셀렉터/대기/파일 형식(PDF vs 엑셀)을 보정해야 함.

1. Chrome 실행 → 근로복지공단 접속 (`total.comwel.or.kr`)
2. 사용자 공동인증서 수동 로그인 (사무대행 151-86-01316)
3. 수임처별 순차 처리:
   - **사업장 전환**: 관리번호(사업자번호+`'0'`) 정확일치 매칭 — `biz_to_mgmt_no()`
     규칙 그대로 적용(NPS와 동일). blind row=0 금지.
   - **메뉴 이동** (`navigate_to_support_info`): 정보조회 → 보험료정보 조회 →
     부과고지 보험료 조회(**코드 20209**) → 하단 **고용** 탭 → **고용보험료 지원금 정보**
   - **인쇄물 다운로드** (`download_support_info_printout`): "인쇄하기" 클릭 →
     CDP 다운로드 → 매직으로 PDF/엑셀 형식 판별 → `고용보험료지원금정보_{YYYYMM}.{pdf|xlsx}` 리네임
4. 저장: `~/Desktop/고용보험_{YYYYMM}/{수임처}/`
