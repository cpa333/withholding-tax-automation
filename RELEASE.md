...
### 사후 분석: "자동 업데이트가 적용되지 않던" 3중 결함 (2026-07-21 라이브 E2E 로 확정·수정)

v1.0.3 실기기에서 업그레이드 전 과정을 재현하며 확인한 결함과 수정:

1. **무인설치 불발 — 근본 원인 (v1.0.3~v1.1.1 전 배포본 공통)**: 설치기 재실행 래퍼가
   `Popen(["cmd","/c",inner])` 리스트 형태라 `list2cmdline` 이 내부 따옴표를 `\"` 로
   이스케이프 → cmd.exe 는 백슬래시 이스케이프를 해석하지 못해 설치기 경로가 깨짐.
   자동 업데이트가 **다운로드·sha256 검증까지 끝내고 설치기만 조용히 미실행**
   (spawn 은 Popen 성공이라 ok — Inno `/LOG` 파일 미생성으로 확증). v1.1.2 에서
   `cmd /s /c "..."` **문자열** 로 수정. ★ 결함은 구버전 설치본 안에 있어 소급 불가 —
   **v1.1.2 이상을 각 PC 에 1회 수동 설치**해야 이후 자동 업데이트가 정상 동작한다.
2. **설치기 Chrome 감지 오탐 (v1.1.1 이하)**: `Program Files (x86)` 경로 Chrome 을
   미검사 → 해당 PC 에서 무인설치가 게이트에서 exit 1. v1.1.1 릴리스 후 발견,
   v1.1.2 설치기부터 x86 경로 + App Paths 레지스트리 폴백 포함 (`installer.iss`).
3. **업데이트 프롬프트 글자 안 보임 (v1.0.4 이하)**: 시스템 다크모드에서 QApplication
   라이트 팔레트 강제 누락 → 모달이 어두운 배경+검은 글씨. v1.0.5(b0a23b0)에서 기 수정.

검증 기록: v1.0.3(실설치) → 프롬프트/다운로드/검증 정상 → 래퍼 결함 재현 →
수정 래퍼로 동일 다운로드본 spawn 시 +6s Inno 로그 생성 → 무인 설치 → +39s 자동
재실행 확인. v1.1.2→v1.1.3 은 완전 무인 풀사이클로 검증.

재검증 도구: 구버전이 설치된 PC 에서 `python tools/verify_auto_update.py <구버전> <신버전>`
— 실행→프롬프트 자동 클릭→무인설치→재실행까지 무인 관찰 (상세는 스크립트 docstring).
