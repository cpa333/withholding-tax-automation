...
### EI 병합 공식 (2026-07-19 정정)
고용지원금 raw(`고용보험료지원금정보_{YYYYMM}.xls`)의 실업급여지원금/환수금(근로자)이
WEHAGO 업로드 엑셀 "고용보험" 컬럼에 반영:

```
adjustment = -abs(실업급여지원금) + abs(실업급여환수금)
최종값     = 재계산값(셀 기존값) + adjustment      # adjustment != 0 일 때만 기록
```

기준값은 **WEHAGO 재계산 결과**다. 워크플로우가 재계산 → 다운로드 → 병합 → 업로드
순서이므로, 병합 시점에 "고용보험" 셀에는 이미 산정된 보험료가 들어 있다.
`adjustment == 0` 이면 재계산값을 그대로 보존한다.
구현: `_apply_ei_row` (`src/utils/data_merger.py`).

> **정정 이력**: 초기 구현은 `최종값 = adjustment` 로 **덮어써서** 재계산값이 소실됐다
> ("업로드 후 WEHAGO 가 기본 0.9% 를 자동 산정한다"는 전제가 실제 흐름과 달랐음).
> 라이브 실측(2026-07-19, 202606): 이민희 재계산 21,440 → 업로드 **−16,450** 으로 기록됨.
> 올바른 값은 `21,440 − 16,450 = 4,990`.
