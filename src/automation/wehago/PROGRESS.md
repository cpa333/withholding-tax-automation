...
- **"마감"** → 현재 마감 안 됨 → 클릭하여 마감 적용
  - **2단계 모달 처리** (핵심 수정):
    1. **유의사항 안내 모달**: "원천징수 신고각종 신고관련 DATA의 적법성..." 안내
       - 버튼: `취소(esc)`, **`확인(enter)`** (단순 `확인`이 아님!)
       - 기존 코드는 "마감" 텍스트가 포함된 모달만 찾아서 이 모달을 놓침
       - 수정: `_isDialog`에서 열린 모달을 찾아 `확인(enter)` 또는 `확인` 버튼 클릭
    2. **"마감 완료!" 후속 모달**: 마감 성공 안내
       - 버튼: `확인`
       - 추가 클릭 필요
  - 마감 후 버튼이 `마감해제`로 변경되었는지 확인
- **"마감해제"** → 현재 마감 됨 → **자동으로 마감 해제 적용** (과거: 스킵)
  - "마감해제" 버튼 클릭 → 마감 적용과 동일한 2단계 확인 모달 패턴 처리
    (`_click_dialog_confirm` 헬퍼 재사용: `확인(enter)` / `확인` 폴링 클릭)
  - **검증**: 버튼이 `마감해제` → `마감` 으로 전환되어야 함. 전환되지 않으면
    `RuntimeError` 발생 → 어댑터(`wehago_swta.py`)의 try/except 경유로 해당 잡
    실패 처리(`mark_failed`). 사용자가 재점검 가능.
  - 동기: 메뉴 7번 실행 시 이미 마감된 수임처도 재처리를 위해 마감을 풀어주도록
    요구사항 변경.
...
- 등장 시점: 파일 선택 직후 또는 후속 모달 2 "연결되지 않은 사원" 확인 직후
- 수임처별로 사원코드연결 모달이 뜨는 경우와 안 뜨는 경우가 있음
- dismiss_dialogs: "닫기(Esc)" 버튼 매칭 추가 (급여대장 일괄인쇄 등)
- PDF 다운로드 후 급여대장 일괄인쇄 모달 자동 닫기 추가

### SWTA0101 마감 모달 처리 수정
- 기존: "마감" 텍스트가 포함된 z-index overlay만 탐색 → 실제 모달은 "원천징수 신고...유의사항 안내"라 미감지
- 기존: `확인` 버튼만 검색 → 실제 버튼은 `확인(enter)`
- 기존: 대기 1초 + "마감 완료!" 후속 모달 처리 누락
- 수정: `_isDialog`에서 열린 모달을 찾아 `확인(enter)` / `확인` 모두 매치
- 수정: 2단계 모달 처리 (유의사항 안내 → 마감 완료)
- 수정: 모달 대기 15회(0.5초 간격), 후속 모달 5회 처리
- 수정: 마감 후 버튼 상태 검증 (`마감` → `마감해제` 변경 확인)

### 로그인 대기 개선
- 기존: 5초마다 새로고침 → 로그인 입력 중간에 페이지가 리로드되어 불편
...
- **원인 4**: 마감 완료 상태('해제' 버튼)에서는 엑셀 업로드가 비활성화됨
...
- **수정**: 마감 상태 판별 로직 — 네비게이션 바 버튼 텍스트로 판별:
