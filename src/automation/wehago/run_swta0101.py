...
기간 설정:
  ...
  - 반기: 대상 월(GUI year/month 우선, 없으면 현재) 기준 — 7월→당해 상반기(1~6월),
    1월→전년 하반기(7~12월). compute_half_period() 사용.
    ★반기는 1·7월만 실행(그 외 월은 마감 스킵 — 단 라디오 확정값은 역충전).
...
def half_period_target(year, month):
    ...
    반기 신고는 1·7월만 해당 → 그 외 월은 skip=True (마감하지 않음).
    ...
    if year is not None and month is not None:
        ...
        target = datetime.now()
    return target, target.month not in (1, 7)
...
async def run_swta0101(page, year: int = None, month: int = None,
    ...
    if cycle == "매월":
        ...
        # 반기 신고는 연 2회: 7월→상반기(당해 1~6월) / 1월→하반기(전년 7~12월).
        # 대상 시점: GUI year/month 우선, 없으면 현재. ★1·7월만 실행, 나머지 스킵.
        target, skip = half_period_target(year, month)
        if skip:
            log(f"[SWTA0101] 반기 → 비신고월({target.month}월) 마감 스킵 (반기는 1·7월만)")
    ...
    # [4-1] 정보성 게이트 모달 → 확인 (텍스트 단일 매치 → 다중 단어 확장)
    for _ in range(3):
        ...
        if loaded:
            ...
            break
    # [5] 진입 전 잔여 모달 재처리(마감 버튼이 모달에 가려 렌더 안 되는 것 방지).
    await dismiss_dialogs(page)
