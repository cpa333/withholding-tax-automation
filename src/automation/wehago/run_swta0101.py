...
조회 시 확인/취소 선택 모달 처리:
  - 최초 조회·재조회 **모두 "취소"** 를 누른다. 확인을 누르면 저장된(마감 시점)
    내용이 다시 올라와 새로 산출된 급여자료가 반영되지 않는다.
  - dismiss_dialogs() 는 모달을 가리지 않고 '확인'을 누르므로(`_common.py`),
    취소 처리를 반드시 그보다 먼저 수행한다.
...
async def _click_choice_dialog_cancel(page, max_polls: int = 6,
                                      interval: float = 0.5) -> str | None:
    """확인/취소 선택 모달에서 **취소** 를 클릭.

    마감해제 후 재조회 시 "저장된 내용을 불러오시겠습니까?" 류의 모달이 뜨는데,
    확인을 누르면 **마감 시점에 저장된 내용**이 다시 올라와 옛 데이터로 재마감된다.
    취소를 눌러야 새로 산출된 급여자료가 반영된다.

    확인·취소가 **둘 다** 있는 양자택일 모달만 대상으로 한다(단일 확인 모달은
    건드리지 않음). 주의: dismiss_dialogs() 는 모달을 가리지 않고 '확인'을
    누르므로(`_common.py`) 반드시 그보다 **먼저** 호출해야 한다.

    Returns: 클릭한 모달의 텍스트(앞 80자) 또는 None.
    """
    for _ in range(max_polls):
        await asyncio.sleep(net_mult(interval))
        hit = await page.evaluate("""() => {
            const FRAGMENTS = ['저장된 내용', '이전에 신고', '신고서가 있',
                               '이미 신고', '불러오'];
            const dialogs = document.querySelectorAll('._isDialog, .LUX_basic_dialog');
            const pick = [];
            for (const d of dialogs) {
                ...
                if (cs.display === 'none' || cs.visibility === 'hidden'
                    || d.offsetWidth < 50) continue;
                const btns = [...d.querySelectorAll('button')].filter(b => b.offsetWidth > 0);
                const cancel = btns.find(b => b.textContent.trim().startsWith('취소'));
                const ok = btns.find(b => {
                    const t = b.textContent.trim();
                    return t === '확인' || t === '확인(enter)';
                });
                if (!cancel || !ok) continue;
                const txt = d.textContent.trim().replace(/\\s+/g, ' ');
                pick.push({cancel, txt, known: FRAGMENTS.some(f => txt.includes(f))});
            }
            if (!pick.length) return null;
            const target = pick.find(p => p.known) || pick[0];
            target.cancel.click();
            return target.txt.slice(0, 80);
        }""")
        if hit:
            return hit
    return None
...
async def _click_search_and_dismiss(page, warn_if_missing: bool = False) -> None:
    """조회 버튼 클릭 → 대기 → 선택 모달 취소 → 잔여 모달 정리.

    #Search 영역의 가시 "조회" 버튼을 클릭하고, 로딩 대기(sleep 5) 후
    확인/취소 양자택일 모달은 **취소**, 나머지 정보성 모달은 닫는다.
    최초 조회와 마감해제 후 재조회 양쪽에서 공통으로 사용.

    확인을 누르면 저장된(마감 시점) 내용이 올라와 새 급여자료가 반영되지
    않으므로, 미마감·마감 여부와 무관하게 항상 취소를 누른다.

    Args:
        warn_if_missing: True 면 선택 모달을 찾지 못했을 때 WARN 로그를 남긴다.
            마감해제 후 재조회는 모달이 반드시 떠야 하므로 True.
            최초 조회는 수임처 상태에 따라 모달이 없을 수 있어 False(로그 생략).
    """
    ...
    # 확인/취소 양자택일 모달 → 취소.
    # dismiss_dialogs 가 '확인'을 눌러버리기 전에 반드시 먼저 처리해야 한다.
    cancelled = await _click_choice_dialog_cancel(page)
    if cancelled:
        log(f"  선택 모달 → 취소 (저장된 내용 불러오지 않음): {cancelled}")
    elif warn_if_missing:
        log("  WARN: 확인/취소 선택 모달을 찾지 못함 — 저장된 내용이 "
            "그대로 반영될 수 있음 (모달 문구 변경 여부 확인 필요)")

    # 조회 직후 정보성 모달(저장된 내용 / 이전에 신고한 신고서 등) 공통 1차 닫기.
    await dismiss_dialogs(page)

    # 정보성 게이트 모달 → 확인 (텍스트 단일 매치 → 다중 단어 확장)
    # 위에서 선택 모달을 취소했다면, 같은 모달에 '확인'을 눌러 되돌리는 것을 막기 위해 건너뛴다.
    for _ in range(0 if cancelled else 3):
...
async def run_swta0101(page, year: int = None, month: int = None,
    ...
    if btn_text == "마감":
        ...
        # 재조회 시 뜨는 확인/취소 모달은 **취소**. 확인을 누르면 마감 시점에
        # 저장된 내용이 올라와 새 급여자료가 반영되지 않는다.
        log("  마감해제 완료 — 해당 월로 재조회(선택 모달 취소) 후 재마감 진행...")
        await _click_search_and_dismiss(page, warn_if_missing=True)
